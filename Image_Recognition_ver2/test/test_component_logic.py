...
class ComponentLogicTests(unittest.TestCase):
    ...
    @staticmethod
    def _fake_realsense(devices):
        class FakeDevice:
            def __init__(self, name, serial):
                self._values = {"name": name, "serial": serial}

            def get_info(self, key):
                return self._values[key]

        fake_devices = [FakeDevice(name, serial) for name, serial in devices]
        return SimpleNamespace(
            camera_info=SimpleNamespace(name="name", serial_number="serial"),
            context=lambda: SimpleNamespace(query_devices=lambda: fake_devices),
        )

    def test_builder_ports_match_startup_generation_interface(self):
        self.assertEqual(self.component._camera_serial, [""])
    ...
    def test_camera_auto_selection_accepts_d435(self):
        rs = self._fake_realsense(
            [
                ("Intel RealSense D455", "900"),
                ("Intel RealSense D435", "200"),
            ]
        )

        self.assertEqual(
            self.component._select_supported_camera(rs, ""),
            ("200", "Intel RealSense D435"),
        )

    def test_camera_serial_selection_accepts_d435i(self):
        rs = self._fake_realsense(
            [
                ("Intel RealSense D435", "200"),
        ...
        self.assertEqual(
            self.component._select_supported_camera(rs, "300"),
            ("300", "Intel RealSense D435I"),
        )

    def test_camera_selection_rejects_other_models(self):
        rs = self._fake_realsense(
            [
                ("Intel RealSense D455", "900"),
                ("Intel RealSense D435F", "901"),
            ]
        )

        with self.assertRaisesRegex(RuntimeError, "No RealSense D435 or D435i"):
            self.component._select_supported_camera(rs, "")
    ...
    def test_preview_shortcut_queues_only_one_enabled_target_while_idle(self):
        ...
        for target_id in ("[T1]", "[T2]", "[T3]", "[T4]"):
            with self.subTest(target_id=target_id):
                ...
                self.assertTrue(self.component._queue_preview_target(target_id))
                self.assertEqual(list(self.component._pending_target_ids), [target_id])
