...
class Throughput_py(OpenRTM_aist.DataFlowComponentBase):
    ...
    def receiveData(self, tm, seq_length):
        ...
        if self._size != seq_length and record_num != 0:
            ...
            throughput = ((((self._size * self._varsize) + 8)
                           * 8) / mean_latency) / (1024 * 1024)
    ...
    def setConnectorProfile(self, info):
        outputfile = self._datatype[0] + "-" + info.properties.getProperty(
            "interface_type") + self._filesuffix[0] + ".dat"
        if self._fs is None:
            try:
