...
class Image_Recognition(OpenRTM_aist.DataFlowComponentBase):
    ...
    def __init__(self, manager):
        ...
        """
         ...
         - Name:  camera_serial
         - DefaultValue: (automatic D435/D435i selection)
        """
        self._camera_serial = ['']
        """
    ...
    def onInitialize(self):
        ...
        self.bindParameter("camera_serial", self._camera_serial, "")
    ...
    #
    @staticmethod
    def _device_info(device, info_key, fallback="unknown"):
        try:
            value = str(device.get_info(info_key)).strip()
            return value or fallback
        except Exception:
            return fallback

    @staticmethod
    def _is_supported_camera_name(name):
        model_tokens = str(name).upper().replace("-", " ").split()
        return any(token in {"D435", "D435I"} for token in model_tokens)

    def _select_supported_camera(self, rs, requested_serial):
        """Select one connected D435 or D435i, optionally by serial number."""

        devices = list(rs.context().query_devices())
        connected = []
        supported = []
        for device in devices:
            name = self._device_info(device, rs.camera_info.name)
            serial = self._device_info(device, rs.camera_info.serial_number)
            connected.append((serial, name))
            if self._is_supported_camera_name(name):
                supported.append((serial, name))

        requested = str(requested_serial).strip()
        if requested:
            matching = [item for item in connected if item[0] == requested]
            if not matching:
                available = ", ".join(
                    "{} ({})".format(name, serial) for serial, name in connected
                ) or "none"
                raise RuntimeError(
                    "Configured RealSense serial {} is not connected; available: {}".format(
                        requested, available
                    )
                )
            serial, name = matching[0]
            if not self._is_supported_camera_name(name):
                raise RuntimeError(
                    "Configured RealSense {} ({}) is not a D435 or D435i".format(
                        name, serial
                    )
                )
            return serial, name
        ...
        if not supported:
            ...
            raise RuntimeError(
                "No RealSense D435 or D435i is connected; available: {}".format(
                    available
                )
            )
        ...
        if len(supported) > 1:
            LOGGER.warning(
                "Multiple D435/D435i cameras are connected; automatically using %s (%s). "
                "Set camera_serial to select a different device.",
                supported[0][1],
                supported[0][0],
            )
        return supported[0]
    ...
    def onActivated(self, ec_id):
        ...
        try:
            ...
            serial, camera_model = self._select_supported_camera(
                rs, self._camera_serial[0]
            )
            ...
            LOGGER.info(
                "Activated depth-geometry recognition with %s (serial %s)",
                camera_model,
                serial,
            )
            return RTC.RTC_OK
            ...
            if self._active_target_id is not None:
                ...
                if detection is not None:
                    ...
                    LOGGER.info(
                        ...
                        "face shape %s, rectangularity %.3f, "
                        ...
                        *detection.observed_dimensions_mm,
                        ...
                        (
                            "{:.3f}".format(
                                detection.visible_face_shape_confidence
                            )
                            if detection.visible_face_shape_confidence is not None
                            else "disabled"
                        ),
    ...
    def _show_camera_preview(self, color_image, now):
        ...
        try:
            ...
            if self._active_target_id is None and self._pending_target_ids:
                ...
                status = "Waiting: send t1-t4 or press 1-4"
            else:
