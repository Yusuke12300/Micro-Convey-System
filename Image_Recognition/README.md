...
入力メッセージ1件を1回の認識要求として扱います。同じIDを再送した場合も新しい要求です。画像処理が重い環境でも5回の安定判定を完了できるよう、認識要求は最大15秒待機します。15秒以内に信頼できる候補が得られない場合、誤座標は出力しません。`target_point`のXYZは、変換後のメートル単位の浮動小数点値を丸めずに送信します。
