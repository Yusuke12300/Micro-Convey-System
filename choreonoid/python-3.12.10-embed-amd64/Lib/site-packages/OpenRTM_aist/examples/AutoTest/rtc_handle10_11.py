...
class InPipe():
    def __init__(self, comp, port):
        ...
        self.pipe = comp.makeOutPort(self.pname, port.data_class(
            RTC.Time(0, 0), []), OpenRTM_aist.RingBuffer(1))
...
class OutPipe():
    def __init__(self, comp, port):
        ...
        self.port = port
        self.pname = port.name.replace('.', '_')
        self.pipe = comp.makeInPort(self.pname, port.data_class(
            RTC.Time(0, 0), []), OpenRTM_aist.RingBuffer(1))
        self.buf = getattr(comp, '_d_' + self.pname)
        tmp = self.pipe.getPortProfile()
